...
if __name__ == '__main__':
    ...
    zip_path = r'E:\luding\Zip_dir'  # 加载压缩包路径
    EOF_dir = r'E:\luding\OPOD_dir'  # 加载精轨数据路径
    ...
    if os.path.isdir(E0F_history_dir):
        ...
        Existing_EOF_file = []
    GenerateTable(zip_path, EOF_dir)  # 生成zip文件日期表格
    # RetrievalTable(EOF_dir)  # 生成OPOD_dir中已存在的日期表格
    # targetExcel = EOF_dir + '/datetable.xls'
    # basisExcel = EOF_dir + '/retrievaltable.xls'
    # field = ''
    # outputpath = EOF_dir
    #
    # df_bE = pd.read_excel(basisExcel)
    # if df_bE.empty and len(df_bE.columns) == 0:
    #     temp_dir = os.path.dirname(targetExcel) + '/tmp'
    #     if not os.path.exists(temp_dir):
    #         os.makedirs(temp_dir)
    #     if os.path.exists(basisExcel):
    ...
    #         os.remove(basisExcel)
    # else:
    #     removeRepeat(targetExcel, basisExcel, field, outputpath)  # 对比以上表格，从A-B获得结果表
